...
class Pipeline:
    ...
    def update_file_names(self, data_path, lanes_file, path_out, image_file=None, bf_files=None, nuc_files=None):
        ...
        if (self.n_fovs is not None) and (self.n_fovs!=self.n_fovs_lanes): #needs adaptation if no image_file
            n_fovs_min = min(self.n_fovs, self.n_fovs_lanes)
            self.n_fovs = n_fovs_min
            self.fovs = list(range(self.n_fovs))
            self.n_fovs_lanes = n_fovs_min
            self.fovs_lanes = list(range(self.n_fovs_lanes))
        ...
        # if self.n_fovs != self.n_fovs_lanes:
        #     print('The number of fovs in the lanes file is not equal to the number of fovs in the nd2 file. Please check if the files are correct. If it is, make sure to specify the fovs and lanes_fovs in the parameters.')
    ...
    def check_files(self, data_path, lanes_file, path_out, image_file=None, bf_files=None, nuc_files=None):
        ...
        if image_file is not None:
            ...
            n_fovs = None
            self.tres = None
            print('Warning: image_file is set to None. Might not be able to continue with pipeline.')
            cell_images_checked = True
        ...
        if (n_fovs is not None) and (n_fovs!=n_fovs_lanes):
             print(f'WARNING! The number of fovs in the lanes file ({n_fovs_lanes}) is not equal to the number of fovs in the nd2 file ({n_fovs}).' +
                   f' Please check if the files are correct. If it is, make sure to specify the fovs and lanes_fovs in the parameters.' +
                   f' Will continue with {min(n_fovs, n_fovs_lanes)} as number of fovs for both files.')
        else:
    ...
    def infer_pixel_micron_ratio(self):
        ...
        if self.nd2_cell_files:
            ...
            if 'pixel_microns' in f.metadata.keys():
                ...
                self.pixelperum = 1 / f.metadata['pixel_microns'] # changed 06/06/2024. Before it was accidently um per pixel
            else:
    ...
    def detect_lanes(self, fovs, verbose=True):
        ...
        for fov in tqdm(fovs, desc='Detecting lanes', disable=not verbose):
            ...
            try:
                ...
                lanes_mask, lanes_metric = lane_detection.get_lane_mask(
                    ...
                    threshold=self.lane_threshold, low_clip=self.lane_low_clip, high_clip=self.lane_high_clip, lanes_dir=lanes_dir)
        ...
        for bad_fov in bad_fovs:
            fovs.remove(bad_fov)
                

        return lanes_mask, lanes_metric, fovs
    ...
    def classify_tracks(self, fov):
        ...
        coarsen = math.ceil(5*60/self.tres)
        # smooth to an hour
        sm = math.ceil(60*60/(self.tres*coarsen))
        #set minimum length
    ...
    def creat_directories_for_output(self): #correct typo and references -> create_...
        """
    ...
    def convert_pixels_to_um(self, fov):
        ...
        df.insert(len(df.columns), 'nucleus_um', df.nucleus/self.pixelperum)
        df.insert(len(df.columns), 'rear_um', df.rear/self.pixelperum)
        df.insert(len(df.columns), 'front_um', df.front/self.pixelperum)
        df.insert(len(df.columns), 'v_nuc_um', df.v_nuc/self.pixelperum/self.tres)
        df.insert(len(df.columns), 'v_rear_um', df.v_rear/self.pixelperum/self.tres)
        df.insert(len(df.columns), 'v_front_um', df.v_front/self.pixelperum/self.tres)
        df.insert(len(df.columns), 'length_um', df.length/self.pixelperum)
        df.insert(len(df.columns), 'area_um', df.area/self.pixelperum**2)
    ...
    def run_pipeline(self, run_segmentation=True, run_tracking=True, run_lane_detection=True, merge_trajectories=True, classify_trajectories=False, verbose=True, convert_to_um=True):
        ...
        if run_lane_detection:
            self.fovs = self.detect_lanes(self.fovs)[2]
        ...
        for fov in self.fovs:
            ...
            ## Now convert to um
            # Buggy. view_all_exps plots images with pixel dimensions and converted arrays with um values. Doesn't add up.
            if convert_to_um:
