...
class Viewer(Pipeline):
    ...
    def __init__(self, pipeline, fovs=None, fovs_lanes=None, frame_indices=None):
        ...
        # Should this really be pixelperum or um per pixel??
        ...
        #slider for the fovs and make them as wide as the image
        ...
        elif self.fovs_lanes is not None:
            self.fov = widgets.IntSlider(min=min(self.fovs_lanes),max=max(self.fovs_lanes), step=1, description="fov", continuous_update=False)
        #slider for the frames and make them as wide as the image
    ...
    def update(self, fov, clip, frame):
        ...
        if fov not in self.fovs or not frame in self.frame_indices:
            ...
            self.button_delete_fov.description='Add fov'
    ...
    def delete_fov(self, a):
        ...
        """
        if self.fov.value in self.fovs:
            self.fovs.remove(self.fov.value)
            self.fovs_lanes.remove(self.fov.value)
            self.fov.value+=1
            self.update(self.fov.value, self.clip.value, self.frame.value)
        else:
            self.fovs.append(self.fov.value)
            self.fovs_lanes.append(self.fov.value)
            # return
    ...
    def update_pixelperum(self, a):
        ...
        if self.select_pixelperum.value is None:
            ...
            return
        try:
            self.pixelperum = float(self.select_pixelperum.value)
        except TypeError:
...
class ResultsViewer:
    ...
    def __init__(self, nd2file, outpath, base_path=None, experiment_paths=[], db_path = '/project/ag-moonraedler/MAtienza/database/onedcellmigration.db', path_to_patterns=None):
        ...
        dir_list = os.listdir(self.outpath)
        if dir_list is not None:
            fov_list = [int(fov.strip()[2:]) for fov in dir_list if 'XY' in fov]
            if fov_list != []:
                v_min = min(fov_list)
                v_max = max(fov_list)

        self.v = widgets.IntSlider(min=v_min, max=v_max, step=1, description="v", continuous_update=False)
    ...
    def onclick(self, event):
        ...
        # tres = 30 # now the viewer gets the correct value itself during init
        def t_to_frame(t):
            return t / (self.tres / 60)
        def frame_to_t(frame):
            return frame * self.tres / 60
