import numpy as np
import os
import cupy as cp # deactivated for jupyter hub
...
def get_lane_mask(image, kernel_width=5, line_distance=30, threshold=0.5, scaling=0.25, low_clip=0, high_clip=500, 
                  debug=False, gpu=True, lanes_dir=None):
    ...
    Args:
        ...
        lanes_dir (str, optional): specify where lanes_widths should be saved. Workaround. Fix in a later release.
    
    Returns:
        Mask image containing 0s where there is no lane, and a different integer for every separate line.

    Comment:
        function of the same name exists in functions.py. Solve this conflict.
        A lane class might be advisable to be able to access certain properties more easily such as lane widht, distance between lanes, fluorescent intensity, etc.
    """
    ...
    np.savetxt(os.path.join(lanes_dir,'lane_widths.txt'), lane_widths, delimiter=",")
